...
#define p_sign(m) ((int)(m))
#define p_cast(m) ((int)(m))
...
bool p_check(Poly *p)
{
  ...
  for (compare=1; compare && compare != sum; compare *= (int)(-2))
  {
    ;
  }
...
#define bp_sprintf(...) {\
  ...
  for (first = 1, pt = p->term, pend = pt+p->len; pt != pend; ++pt)
  {
    ...
    m = p_sign(pt->m);
    l = p_sign(pt->l);
    if (pt->coef < ((int)0)) bp_sprintf(" - %n")
    else if (!first) bp_sprintf(" + %n")
    if (pt->coef > ((int)1) ) bp_sprintf("%ld%n", pt->coef)
    else if (pt->coef < (int)(-1)) bp_sprintf("%ld%n", -pt->coef)
    else if ((!l) && (!m)) bp_sprintf("%d%n", 1)
    if (pt->m)
    {
      //bp_sprintf("M%n")   //用z替代M
      bp_sprintf("z%n")
      if (m != 1) bp_sprintf("^%ld%n", m)
    }
    if (pt->l)
    {
      //bp_sprintf("L%n") //用a替代L
      bp_sprintf("a%n")
